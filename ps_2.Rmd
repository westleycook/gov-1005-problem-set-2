...
first_date <- x %>% arrange(date) %>% slice(1) %>% pull(date)

# Ran the arrange() command and figured out what the date was, and then labored
# over it for a long time before Evelyn told me about the slice() command I
# could use to isolate a single row of the dataframe. So I sliced it to keep
# just the first row, then pulled the value from the date column and named it
# first_date so I could slot it in nicely for the Mad Lib.
...
most_recent_date <- x %>% arrange(desc(date)) %>% slice(1) %>% pull(date)
