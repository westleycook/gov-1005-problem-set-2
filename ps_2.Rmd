...
# plot I'm replicating.
# It then creates the plot, trying to replicate the plot from the
...
# Setting scale for x axis: it'll auto-generate appropriate limits, but I want
# vertical grid lines to appear for years from 1950-2010, so I'll use the breaks
# argument with a seq() command to set the first and last years they'll appear,
# and how much space to put between them (by = 10).
...
# Now will similarly scale and label the y axis. Unlike the x axis, for this one
# I need to set clear limits (the auto-generated limits don't match the plot I'm
# replicating).
...
# Want to add title, subtitle, and caption; using labs() because I can add all
# of those within the same operation using some of the lab() arguments.
# Character strings for title, subtitle, and caption all copied from the plot
# I'm replicating; labels are NULL because the plot didn't have any.
...
# Adding text on the plot to insert "Democrats" in blue. x and y are the
# coordinates on the plot (figured out what the exact numbers should be for
# those, and for size, through trial and error). Other arguments are
# self-explanatory.
...
# Using this to manually change the color of the lines on the plot from their
# default settings, so the Democrats line will be blue and the Republican line
# will be red.
