...
To set up this pset, you'll need two packages, **tidyverse** (which you should have installed in class), and  **fivethirtyeight**. See [*PPBDS*](https://davidkane9.github.io/PPBDS/1-getting-started.html#package-installation) if you need a reminder on how to install and load packages.
...
# --- is special. By default, it does not print out messages. So, it is a handy
...
# It's nice to keep your answers to each Mad Lib in a separate chunk of code.
# Give the object a name that means something, like ml_1
...
4. Endorsers from New Hampshire have an average of X endorsement points. (Learn how fivethirtyeight calculates endorsement points [here](https://fivethirtyeight.com/methodology/how-our-presidential-endorsement-tracker-works/).)

```{r madlib5, include=FALSE}
# We've learned the summarize command in the Datacamps, but not yet in the
# textbook. Check out Introduction to the Tidyverse Chapter 3 for some tips.
...
congress %>%
  ...
  ggplot(mapping = aes(x = year, y = avg_age, color = party)) +
    ...
    # labeling all the points in your plot. gghighlight is also fun.
